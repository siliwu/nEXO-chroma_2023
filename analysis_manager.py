...
class analysis_manager:
	def __init__(self, geometry_manager, photons, photon_tracks = None, run_id = 0, seed = 0, histories = None):
		...
		self.plot_tracks(num_tracks = num_tracks, detected_only = True, reflected_only = False)
		...
		# self.plot_filtered_tracks(filter_by = 3, num_tracks = num_tracks)
		...
		# self.photon_incident_angle_emission_angle_correlation(num_tracks = None, detected_only = True, reflected_specular_only = True, reflected_diffuse_only = False)
	...
	def save_detected(self, filename):
		...
		# cu_eta = self.gm.mat_manager.material_props['copper']['eta']
		# cu_k = self.gm.mat_manager.material_props['copper']['k']
		# al_eta = self.gm.mat_manager.material_props['aluminum']['eta']
		# al_k = self.gm.mat_manager.material_props['aluminum']['k']

		# Cu_eta_index = self.gm.surf_manager.eta2
		# Cu_k_index = self.gm.surf_manager.k2
	    # print(self.gm.surf_manager.eta2)
		...
		save_data = {
				...
				# 'Cu eta': np.array([Cu_eta_index for _ in range(len(self.detected_angles))]),
				# 'Cu k': np.array([Cu_k_index for _ in range(len(self.detected_angles))])}
...
#Sili: added on 02/07/2023 to plot the shooting angle and emission angle correlation of detected photons
	def photon_incident_angle_emission_angle_correlation(self, num_tracks = None, detected_only = True, reflected_specular_only = False, reflected_diffuse_only = False):
		if num_tracks == None:
			num_tracks = self.num_particles
	...
	def plot_tracks(self, num_tracks, detected_only = True, reflected_only = True, diffuse_only = False):
		if num_tracks == None:
			num_tracks = self.num_particles
			print(num_tracks)
